...
public class SensorLluvia {
    ...
    public static void main(String[] args) {
        HiloSensadoLluvia hilo = new HiloSensadoLluvia(SensorLluvia::abrirConexionConLider);
        hilo.start();
    }
    ...
    private static ConexionLider abrirConexionConLider() throws Exception {
        ...
        System.out.printf("[SENSOR-L] Conectado al líder en %s:%d%n", info.host(), info.puerto());
        return new ConexionLider(socket, out, info.host(), info.puerto());
    }
    ...
    private static LiderInfo obtenerInfoLider() {
        ...
        try {
            ...
            System.out.printf("[SENSOR-L] Líder actual: %s:%d%n", host, puerto);
        } catch (Exception e) {
            System.err.printf("[SENSOR-L] No se pudo obtener líder actual (%s), usando %s:%d%n", e.getMessage(), host,
                    puerto);
        }
