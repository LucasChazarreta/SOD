...
public class HiloSensado extends Thread {
    ...
    private final SensorHumedad2.ConexionProvider conexionProvider;
    private String hostActual = "desconocido";
    private int puertoActual = -1;

    public HiloSensado(SensorHumedad2.ConexionProvider provider){
    ...
    public void run(){
        ...
        while(on){
            try {
                ...
                System.out.printf("[SENSOR-H2] Humedad: %.1f%% -> %s:%d%n", humedad, hostActual, puertoActual);
                ...
                System.err.println("[SENSOR-H2] Error enviando datos al líder: " + e.getMessage());
    ...
    private void asegurarConexion() throws Exception {
        ...
        if (cnxServidor != null) {
            System.out.printf("[SENSOR-H2] Conexión perdida con %s:%d, reintentando...%n", hostActual, puertoActual);
        }

        cerrarConexionActual();
        SensorHumedad2.ConexionLider nuevaConexion = conexionProvider.nuevaConexion();
