...
public class SensorHumedad2 {
    ...
    public static void main(String[] args) {
        HiloSensado sensor = new HiloSensado(SensorHumedad2::abrirConexionConLider);
        sensor.start();
    }
    ...
    private static ConexionLider abrirConexionConLider() throws Exception {
        ...
        pw.println("sensorHumedad;2");
        pw.flush();

        System.out.printf("[SENSOR-H2] Conectado al líder en %s:%d%n", info.host(), info.puerto());
    ...
    private static LiderInfo obtenerInfoLider() {
        ...
        try {
            ...
            System.out.printf("[SENSOR-H2] Líder actual: %s:%d%n", host, puerto);
        } catch (Exception e) {
            System.err.printf("[SENSOR-H2] No se pudo obtener líder actual (%s), usando %s:%d%n", e.getMessage(), host,
                    puerto);
        }
