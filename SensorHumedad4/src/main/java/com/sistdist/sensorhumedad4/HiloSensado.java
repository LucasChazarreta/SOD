...
public class HiloSensado extends Thread {
    ...
    private final SensorHumedad4.ConexionProvider conexionProvider;
    private String hostActual = "desconocido";
    private int puertoActual = -1;

    public HiloSensado(SensorHumedad4.ConexionProvider provider){
    ...
    public void run(){
        ...
        while(on){
            try {
                ...
                System.out.printf("[SENSOR-H4] Humedad: %.1f%% -> %s:%d%n", humedad, hostActual, puertoActual);
                ...
                System.err.println("[SENSOR-H4] Error enviando datos al líder: " + e.getMessage());
    ...
    private void asegurarConexion() throws Exception {
        ...
        if (cnxServidor != null) {
            System.out.printf("[SENSOR-H4] Conexión perdida con %s:%d, reintentando...%n", hostActual, puertoActual);
        }

        cerrarConexionActual();
        SensorHumedad4.ConexionLider nuevaConexion = conexionProvider.nuevaConexion();
