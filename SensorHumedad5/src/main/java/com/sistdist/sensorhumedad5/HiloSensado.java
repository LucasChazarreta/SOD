...
public class HiloSensado extends Thread {
    ...
    private final SensorHumedad5.ConexionProvider conexionProvider;
    private String hostActual = "desconocido";
    private int puertoActual = -1;

    public HiloSensado(SensorHumedad5.ConexionProvider provider){
    ...
    public void run(){
        ...
        while(on){
            try {
                ...
                System.out.printf("[SENSOR-H5] Humedad: %.1f%% -> %s:%d%n", humedad, hostActual, puertoActual);
                ...
                System.err.println("[SENSOR-H5] Error enviando datos al líder: " + e.getMessage());
    ...
    private void asegurarConexion() throws Exception {
        ...
        if (cnxServidor != null) {
            System.out.printf("[SENSOR-H5] Conexión perdida con %s:%d, reintentando...%n", hostActual, puertoActual);
        }

        cerrarConexionActual();
        SensorHumedad5.ConexionLider nuevaConexion = conexionProvider.nuevaConexion();
