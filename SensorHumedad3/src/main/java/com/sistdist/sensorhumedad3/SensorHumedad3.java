...
public class SensorHumedad3 {
    ...
    public static void main(String[] args) {
        HiloSensado sensor = new HiloSensado(SensorHumedad3::abrirConexionConLider);
        sensor.start();
    }
    ...
    private static ConexionLider abrirConexionConLider() throws Exception {
        ...
        System.out.printf("[SENSOR-H3] Conectado al líder en %s:%d%n", info.host(), info.puerto());
    ...
    private static LiderInfo obtenerInfoLider() {
        ...
        try {
            ...
            System.out.printf("[SENSOR-H3] Líder actual: %s:%d%n", host, puerto);
        } catch (Exception e) {
            System.err.printf("[SENSOR-H3] No se pudo obtener líder actual (%s), usando %s:%d%n", e.getMessage(), host,
                    puerto);
        }
